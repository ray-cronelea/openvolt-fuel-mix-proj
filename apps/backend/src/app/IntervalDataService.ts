import { IntervalData } from './DomainObjects';
import moment from 'moment/moment';
import axios from 'axios';

export async function getMonthIntervalData(monthVal: Date): Promise<IntervalData> {
  const params = {
...
export async function getHalfHourIntervalData(monthVal: Date): Promise<IntervalData> {
  const params = {
    meter_id: '6514167223e3d1424bf82742',
    granularity: 'hh',
    start_date: moment(monthVal).toISOString(),
